...
async function loadMessages(append = false) {
  ...
  try {
    ...
    messages.forEach(m => {
      ...
      card.id = `msg-${m.id}`;
      card.style.cssText = "background:" + (isRead ? "var(--cream)" : "var(--warm-white)") +
                           ";border:1px solid " + (isRead ? "var(--border)" : "#009E50") +
      ...
      card.innerHTML = `
        <div class="msg-name-row" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.5rem;flex-wrap:wrap;gap:0.5rem;">
          <div style="display:flex;align-items:center;gap:0.6rem;min-width:0;">
            <span class="unread-dot" style="width:8px;height:8px;background:#009E50;border-radius:50%;display:${isRead ? "none" : "inline-block"};flex-shrink:0;"></span>
        ...
        <a class="msg-email-link" style="font-size:0.875rem;color:#00753C;"></a>
        ...
        <div style="margin-top:1rem;display:flex;gap:0.75rem;">
          <button class="toggle-read-btn" onclick="toggleRead(${m.id}, ${isRead})" style="font-size:0.8rem;padding:0.3rem 0.85rem;border-radius:50px;border:1px solid #009E50;background:transparent;color:#00753C;cursor:pointer;">
...
async function toggleRead(id, currentlyRead) {
  ...
  try {
    ...
    card.style.borderColor    = nowRead ? "var(--border)" : "#009E50";
